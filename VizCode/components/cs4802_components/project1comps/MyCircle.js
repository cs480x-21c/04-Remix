...
class MyCircle extends Component {
  ...
  componentDidMount() {
    ...
    d3.select(this.myRef.current).select("#first").attr('transform', 'translate(' + w*0.25 + ',' + h*0.5  + ')');
    d3.select(this.myRef.current).select("#second").attr('transform', 'translate(' + w*0.5 + ',' + h*0.5  + ')');
    d3.select(this.myRef.current).select("#third").attr('transform', 'translate(' + w*0.75 + ',' + h*0.5  + ')');
  }
  ...
  render() {
    return <div className="circleDiv">
              <h5>Refresh Page to change circle size!</h5>
              <div ref={this.myRef}>
              </div>  
           </div>
  }
